...
## 이미지: 실제 Grafana 캡처와 실측 선 그래프

![Grafana에서 InfluxDB로 직접 조회한 4,000명 p95 숫자 화면](./grafana-opening-4000-measurements.jpg)

![Grafana와 같은 k6 원본 실측값으로 만든 단계별 선 그래프](./grafana-measured-slope-comparison.jpg)

두 번째 이미지는 Grafana 캡처의 픽셀에서 값을 다시 읽지 않고, Grafana에 가져온 **동일한 k6 원본 요약값**으로 만들었다. 연결선은 적용 전→후의 개별 집계값 비교이며 시간별 요청 추이를 나타내지 않는다. 50명·4,000명·100만 건은 서로 다른 시나리오로 분리했다.
