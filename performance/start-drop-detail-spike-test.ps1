...
if ($SpikeRps -lt $NormalRps) {
    throw 'SpikeRps must be greater than or equal to NormalRps.'
}
...
$testRun = "drop-detail-spike-$($Phase.ToLower())"
...
$dockerArguments = @(
    ...
    '--env', "DROP_ID=$DropId",
    '--env', "NORMAL_RPS=$NormalRps",
    '--env', "SPIKE_RPS=$SpikeRps",
    '--env', "PRE_ALLOCATED_VUS=$PreAllocatedVUs",
    '--env', "MAX_VUS=$MaxVUs",
    '--tag', "test_run=$testRun",
    '/scripts/drop-detail-spike.js'
)

Write-Host "Running $testRun against Drop $DropId."
Write-Host "Traffic: $NormalRps RPS to $SpikeRps RPS."
